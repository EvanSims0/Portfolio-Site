...
export default function About() {
  ...
  return (
    <section id="about" className="relative py-48 px-6" ref={ref}>
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-3">
              {TECH_STACK.map((tech, i) => (
                <div
                  key={tech.name}
                  className="card-glow border border-surface/30 rounded-none p-5 text-center cursor-default select-none bg-surface-dim/50 border-t-2 border-t-accent/40"
                  style={{
                ...
                >
                  <div className="h-9 flex items-center justify-center mb-3 text-accent">
                    <TechIcon name={tech.name} />
                  </div>
                  <p className="font-body text-sm font-semibold tracking-wide text-text">{tech.name}</p>
                  <p className="font-body text-[11px] text-surface mt-1.5 capitalize">{tech.category}</p>
                </div>
              ))}
            </div>
          </div>
