  ...
  return (
    ...
    >
      ...
      <p className="animate-fade-up delay-600 font-body text-base sm:text-lg text-text-muted max-w-xl mb-12 leading-loose">
        I go to John Brown University to study computer science.
        I've learned that execution is everything from leading student groups to working tech support.
        I care a lot more about shipping useful software than about how cool it sounds.
      </p>
      ...
      <div className="animate-fade-up delay-800 flex flex-wrap gap-4">
        <a
          ...
          className="group inline-flex items-center gap-2 bg-accent hover:bg-accent-dark text-white font-body font-semibold text-sm px-8 py-4 rounded-none transition-all duration-200 shadow-lg shadow-accent/20 hover:shadow-accent/40 hover:gap-3"
        >
          View My Work
        ...
        <a
          ...
          className="inline-flex items-center gap-2 border border-surface/50 text-text-muted hover:border-accent hover:text-accent font-body font-semibold text-sm px-8 py-4 rounded-none transition-all duration-200"
        >
          Download Resume
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {/* Stat bar — value · label  |  value · label */}
      <div className="animate-fade-up delay-700 mt-24">
        <div className="flex flex-wrap items-center gap-y-3">
          {[
            { label: 'Languages',  value: '9+'            },
            { label: 'University', value: 'JBU'           },
            { label: 'Minor',      value: "Int'l Business"},
            { label: 'Degree',     value: 'CS — BA'       },
          ].map(({ label, value }, i) => (
            <div key={label} className="flex items-center font-body">
              {i > 0 && (
                <span className="mx-5 text-surface/35 text-lg font-light select-none">|</span>
              )}
              <span className="text-accent font-semibold text-base">{value}</span>
              <span className="mx-2 text-surface/40 text-sm select-none">·</span>
...
function Chip({ children }) {
  return (
    <span className="inline-block font-mono text-sm bg-surface/15 border border-surface/35 text-text-muted rounded-none px-2 py-0.5">
      {children}
    </span>
  )
}
